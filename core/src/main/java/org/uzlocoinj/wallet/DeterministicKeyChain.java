...
import org.uzlocoinj.core.BloomFilter;
import org.uzlocoinj.core.ECKey;
import org.uzlocoinj.core.NetworkParameters;
import org.uzlocoinj.core.Utils;
import org.uzlocoinj.crypto.*;
import org.uzlocoinj.script.Script;
import org.uzlocoinj.utils.Threading;
 ...
 * {@link org.uzlocoinj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.uzlocoinj.crypto.MnemonicCode},
 ...
 * {@link org.uzlocoinj.crypto.DeterministicKey#serializePubB58(org.uzlocoinj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.uzlocoinj.crypto.DeterministicKey#deserializeB58(org.uzlocoinj.crypto.DeterministicKey, String, org.uzlocoinj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.uzlocoinj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.uzlocoinj.crypto.DeterministicHierarchy} and
 * {@link org.uzlocoinj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final String DEFAULT_PASSUZLASE_FOR_MNEMONIC = "";
    ...
    public static final ChildNumber UZLOCOIN_PATH = new ChildNumber(444,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, UZLOCOIN_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public static class Builder<T extends Builder<T>> {
        ...
        protected String getPassphrase() {
            return passphrase != null ? passphrase : DEFAULT_PASSUZLASE_FOR_MNEMONIC;
        }
    }
    ...
    public DeterministicKeyChain(SecureRandom random) {
        this(random, DeterministicSeed.DEFAULT_SEED_ENTROPY_BITS, DEFAULT_PASSUZLASE_FOR_MNEMONIC, Utils.currentTimeSeconds());
    }
    ...
    public DeterministicKeyChain(SecureRandom random, int bits) {
        this(random, bits, DEFAULT_PASSUZLASE_FOR_MNEMONIC, Utils.currentTimeSeconds());
    }
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_UZLOCOIN_ONLY)
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_UZLOCOIN_ONLY){
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.uzlocoinj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_UZLOCOIN_ONLY;
                break;
            }
        }
        ...
        while (iter.hasNext()) {
            ...
            if (t == Protos.Key.Type.DETERMINISTIC_MNEMONIC) {
                ...
                String passphrase = DEFAULT_PASSUZLASE_FOR_MNEMONIC; // FIXME allow non-empty passphrase
     ...
     * @see org.uzlocoinj.wallet.MarriedKeyChain
     */
